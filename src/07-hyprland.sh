...
ln -s /usr/local/bin/Hyprland usr/local/bin/hyprland
...
mkdir ${project}-wallpaper-${version}-${wallrev}
cd ${project}-wallpaper-${version}-${wallrev}
mkdir -p usr/local/share/hypr/
cp ../assets/install/*.png          usr/local/share/hypr/

mkdir DEBIAN

echo "Source: ${project}-wallpaper
Section: graphics
Priority: optional
Maintainer: Shionn<shionn@gmail.com>
Homepage: https://github.com/hyprwm/${project}
Package: ${project}-wallpaper
Version: ${version}-${wallrev}
Architecture: amd64
Depends: 
Description: Hyprland Wallpapers." >> DEBIAN/control
...
cd .. 
dpkg -b ${project}-wallpaper-${version}-${wallrev}

mv ${project}-wallpaper-${version}-${wallrev}.deb ../
